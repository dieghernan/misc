...
val_atom <-
  filter(prov_enlaces_tab, str_detect(title, "Toledo")) %>% pull(link)
...
val_link <-
  filter(val_enlaces_tab, str_detect(title, "45900")) %>% pull(link)
val_link
...
file_val <- dir_ls("buildings_old", regexp = "45900.building.gml")
...
g2 <- ggplot(buildings_val25) +
  ...
  scale_fill_manual(
    ...
    guide = guide_legend(keywidth = .5, keyheight = 1),
    na.value = "grey50",
    drop = FALSE
  ) +
  theme_void() +
...
ggsave("./buildings_old/toledo.png", g2, width = 7, height = 7)
