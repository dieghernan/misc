...
val_link <-
  filter(val_enlaces_tab, str_detect(title, "MELQUE")) %>% pull(link)
val_link
...
point <- geo("Melque de Cercos, Spain") %>%
  st_as_sf(coords = c("long", "lat"), crs = st_crs(4326)) %>%
  st_transform(25830) %>%
  st_transform(st_crs(buildings_val))

point_bf <- st_buffer(point,   250)

keep <- st_intersects(buildings_val, point_bf, sparse = FALSE)
keep

buildings_val25 <- buildings_val[keep, ]

tile <- mapSpain::esp_getTiles(buildings_val25,"PNOA", zoommin = 2)

plotRGB(tile)
plot(st_geometry(buildings_val25), add=TRUE)

plot(st_geometry(buildings_val25))
...
plot(st_geometry(buildings_val25), axes=TRUE)
...
ggsave("./buildings_old/Melque.png", g2, width = 7, height = 7)
