...
val_atom <-
  filter(prov_enlaces_tab, str_detect(title, "Cádiz")) %>% pull(link)
val_atom
...
val_link <-
  filter(val_enlaces_tab, str_detect(title, "11900")) %>% pull(link)
val_link
...
file_val <- dir_ls("buildings_old", regexp = "11900.building.gml")
...
ggsave("./buildings_old/cadiz.png", g2, width = 7, height = 7)
